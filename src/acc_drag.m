function [a_drag, d_dr, d_dv] = acc_drag(x_ECI, A, m)
...
r_ECI = x_ECI(1:3);
v_ECI = x_ECI(4:6);
h = abs(norm(r_ECI) - R_E(1))*1000; %[m]
if h <= 180e3 % [km] vida, D., Gural, P. S., Brown, P. G., Campbell-Brown, M., & Wiegert, P. (2020). Estimating trajectories of meteors: An observational Monte Carlo approach - I. Theory. Monthly Notices of the Royal Astronomical Society, 491(2), 2688–2705. https://doi.org/10.1093/mnras/stz3160
...
%     H = options.H; % [m] scale height x
    ...
    rho = rho_0*exp(-(h/H)); %[kg/m^3] % density of atmosphere at altitude %% ToDo change this to GD altitude
%         set_rho_atm
...
%         a_drag = -2*c_d*rho*norm(v_rel)^2*(1.21/m)*(m/meteor.pd3d)^(2/3)*unit(v_rel)/1000; % Vida, D., Brown, P. G., & Campbell-Brown, M. (2018). Modelling the measurement accuracy of pre-atmosphere velocities of meteoroids. Monthly Notices of the Royal Astronomical Society, 479(4), 4307–4319. https://doi.org/10.1093/mnras/sty1841
    d_dv = -1/2*c_d*(A/m)*rho*((v_rel*v_rel')/norm(v_rel) + norm(v_rel)*eye(3)); %[m^2/kg]*[kg/m^3]*[m/s] = [1/s]
    drho_dh = -(1/H)*rho_0*exp(-(h/H)); %[kg/m^4]
    dh_dr = r_ECI/norm(r_ECI); %[m/m] Note, let's assume r_ECI comes in [m], then h = norm(r) - R_E so dh_dr = r/norm(r); (We do this so we can avoid the extra 1000 term that appears in the numerator when computing the partial, and in the denominator when converting from m/km to m/m
    drho_dr = drho_dh*dh_dr'; %[kg/km*m^3]
    d_dr = -1/2*c_d*(A/m)*norm(v_rel)*v_rel * drho_dr - d_dv * [0, -omega, 0; omega, 0, 0; 0, 0, 0]; %[1/s^2]
else
